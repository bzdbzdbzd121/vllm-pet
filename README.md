...
| `update` | `{autoCheck:true}` | 启动时自动检查 GitHub 新版本 |
| `window` | 见源码 | 置顶 / 鼠标穿透 / 缩放 / 透明度 / 位置 |

## 自定义皮肤

内置 5 款配色（默认薄荷绿 / 樱花粉 / 海洋蓝 / 暖阳橙 / 石墨黑），设置面板直接切换。
还可以自己做皮肤：在 `userData/skins/<皮肤名>/` 放一个 `skin.json`（可再加 `robot.svg`
整体换造型），设置面板里选择即可生效。完整教程与 SVG class 钩子表：
**[src/renderer/skins/README.md](src/renderer/skins/README.md)**
...
启动时自动检查 GitHub Release 新版本（可在设置里关闭），也可以随时通过托盘菜单
"🔄 检查更新"或设置面板的按钮手动触发。
