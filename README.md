...
- **启动即配置**：首次运行弹出气泡设置面板，填入服务地址即可，右键宠物随时修改
...
首次启动会在宠物旁弹出设置气泡：填入 vLLM 服务地址（如 `http://127.0.0.1:8000`）、
轮询间隔、可选 API Key，保存后立即开始联动。右键宠物可随时重新打开。
...
打 tag 推送到 GitHub 后，`.github/workflows/release.yml` 自动产出全平台安装包：
