...
以下皮肤随应用自带，无需安装，直接在设置里选择（均只改配色，造型与动画一致）：
...
| `maid-robo` | 小V · 女仆机娘 | **全新造型**：白发女仆装 Q 版机娘，头侧机械耳饰 = 状态指示灯 |

内置皮肤的注册方式见 `skin-loader.js` 的 `BUILTIN_SKINS`：换色皮肤只需一个只有
`palette` 的 `skin.json`；造型皮肤（如 `maid-robo`）额外提供自己的 `robot.svg`
（保留 class 钩子即可复用全部状态动画），调色板都会合并到默认皮肤之上。
