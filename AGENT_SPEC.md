...
> **历史文档**：这是项目初建时多代理协作的设计契约，记录了最初的技术选型与共享约定。
> 其中部分细节已演进（例如：粒子特效已全部移入 SVG 内；新增 `stateMap`/`showStatus` 配置；
> 设置改为独立窗口；状态气泡位于宠物下方）。**现行开发规则以 [DEVELOPMENT.md](DEVELOPMENT.md) 为准。**
